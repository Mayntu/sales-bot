...
def _instruction_text() -> str:
    return (
        ...
        "<code>/refresh</code> — удалить себя из БД и начать диалог заново <i>(разрешённые admin chat_id должны быть в .env и на API).</i>\n"
        "<code>/state</code> — состояние, имя, цель, счётчик follow-up, число сообщений.\n""<code>/followup_now</code> — сразу следующий pending follow-up (демо).\n\n"
